...
def on_command(data):
    ...
    if action == 'play':
        ...
        announce_vol = data.get('volume', 150) # Default to 150 if not provided
        ...
        def play_announcement():
            ...
            print(f"Volumen actual: {current_vol}. Iniciando anuncio a volumen: {announce_vol}%...")

            # 1. Fade out current audio logic? 
            # Actually, standard behavior is usually ducking.
            # But here we want to SWAP stream.
            
            # Set Volume for Announcement
            send_mpv_command(['set_property', 'volume', announce_vol])
            
            # 2. Load announcement (append=no replaces current immediately)
            send_mpv_command(['loadfile', value])
            
            # 3. Wait for it to finish? 
            # MPV 'loadfile' is async. We don't know duration easily without querying.
            # But the requirement says "al hablar debe mantener el tono del volumen".
            # If we just loadfile, MPV plays it.
            # After it finishes, does it go back to radio? No, unless we tell it.
            # But currently we don't have a playlist management.
            # However, usually announcements are short.
            # If we want to restore radio after, we need to know when it ends.
            
            # Improved Logic:
            # We assume user will start "Radio" again manually or we just leave it.
            # User said "mantener el tono". 
            # For simplicity and reliability, we set volume, plays file.
